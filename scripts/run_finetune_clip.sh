#!/usr/bin/env bash
# Fine-tune CLIP on train_500 affordance pairs (ranking loss).
set -euo pipefail
...
LOG_FILE="${PROJECT_ROOT}/artifacts/logs/finetune-clip-$(date +%Y%m%d-%H%M%S).log"
echo "Running CLIP fine-tune. Log: ${LOG_FILE}"
